		...
		for(var i = rowCount - 1;i >= 0;i--){
			var checked = Y.one('#check_' + i).get("checked") || undefined;
	  		if(checked){
				...
				if (Y.one('#is_default_' + i).get('checked'))
				{
					tableRow.IS_DEFAULT = "true";
				}
...
YUI().use('node', 'io', 'json-parse', 'json-stringify', 'imes-YuiCommon','node-menunav', function(Y){
	...
	var updateSaveData = function(){
		...
		if(HANDLE != data_field){
			...
			common.setMessage("N", "请先检索工厂" + plant + "下数据字段" + data_field + "数据再进行更新操作");
			return;
		}
		var data = _getData();
		var dataStr = Y.JSON.stringify(data);
		var request = Y.io("${rootPath}/config/datafield/updateDataField.html", { 
			method: "POST", 
			data: "DATA=" + dataStr,
			on: {
	...
	Y.on("click", updateSaveData, "#updateSaveData");//注册按钮点击事件 
	//-------ajax请求数据---------
	...
	var htmlStr = '<input id="selectData" type="button" value="检 索" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' +
				'<input id="updateSaveData" type="button" value="更新保存" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' +
	...
	<div id="menuBar" class="yui3-menu yui3-menu-horizontal yui3-menubuttonnav">
		<div class="yui3-menu-content" style="background-position-y: -1300px;">
	         <ul class="first-of-type">
	            ...
	            <li class="yui3-menuitem" style="font-weight:bold; border-right: 0px;">
	                <a href="#" class="yui3-menuitem-content" style="padding-left: 5px; padding-right: 0px;">更新</a>
	            </li>
	            ...
	            <li class="yui3-menuitem" style="font-weight:bold; float: right; border-right: 0px;">
	                <a href="${rootPath}/config/datafield/gotoPage.html?OPER=SEARCH" class="yui3-menuitem-content">查询</a>
	            </li>
	         </ul>
		</div>
	</div>
