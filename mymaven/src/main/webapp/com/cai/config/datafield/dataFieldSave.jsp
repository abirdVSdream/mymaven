...
YUI().use('datatable', 'datatable-scroll', 'datatype-number', 'node', function (Y) {	
	...
	table = new Y.DataTable({
	    ...
	    { key: 'DATA_VALUE', label: '指定值', width: '15%', allowHTML: true,
	    	formatter: function(o){
	    		var rowCount = table.data.toArray().length;
...
YUI().use('node', 'json-parse', 'json-stringify', function(Y) {
	...
	_getData = function(){
		...
		for(var i = 0;i < rowCount;i++){
			...
			rowData.is_default = Y.one('#is_default_' + i).get("value");
...
YUI().use('node', 'io', 'json-parse', 'json-stringify', 'imes-YuiCommon','node-menunav', function(Y){
	...
	var saveData = function(){
		...
		var request = Y.io("${rootPath}/config/datafield/saveDataField.html", { 
			method: "POST", 
			data: "DATA=" + dataStr,
			on: {
	...
	Y.on("click", saveData, "#saveData");//注册按钮点击事件
	...
	var htmlStr = '<input id="saveData" type="button" value="创建保存" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' +
